c------------------------------------------------------------------------------------
	...
	subroutine conv_cm_3(krad,grav,delta,cp,ro,hp,taur,gradrad,gradad,
...
c ------------------------------------
c Adapté à CRES, D.C. 12 novembre 2002.
c ------------------------------------
...
c	initialisations
	...
	conv=.false.
	!grad=gradad*1.1  <<<<<<<<<<<<<<<<<<<<< Modif. Daniel
	grad=(gradad+gradrad)/2.d0 ! <<<<<<<<<< cette initialisation est plus robuste
                                   ! (cas où Delad légèrement négatif !

	if ( abs(gradad-gradrad) .lt. 1.d-8 ) then ! Modif. Daniel, pour que le code soit plus robuste
	   grad = gradad
	   return
	end if
	...
	phi=1.d30
        !print*, '========================================'
        !print*, 'Dans conv_cm_3 :'
	!print*, ' '
	!print*, ' * gradad  = ', gradad
	!print*, ' * gradrad = ', gradrad
...
c	Newton-Raphson "d" signifie derivee/ grad	
	...
	do while(.not.conv)
	 ...
	 if(iter .gt. 30)then
	  ...
	  write(6,*)phi,phip,abs(phi-phip)/phi
	  print*, 'gradrad = ', gradrad
	  print*, 'gradad  = ', gradad
	  stop
	 endif
