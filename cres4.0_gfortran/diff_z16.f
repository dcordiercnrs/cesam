...
c*************************************************************************
	...
	subroutine diff_z16(melange,p,t,r,l,m,ro,drox,kap,dkapx,
...
c	on a aussi le cas de l'element supplementaire de charge 18 et de masse 27
...
c	on aura de meme ddx(1,1)=ddx(3,1)=0 au lieu de ddx(1,1)=ddx(2,1)=ddx(3,1)
...
c	dvdx(i) : derivee de v / X1
	...
	real*8	p,t,xchim(1),m,l,r,ro,kap,cs(pnelem,2),as(pnelem,2),
     ...
     5	ds(2,pnelem),vs(pnelem),stor,unpdx,dstor,w_diff
	...
	save init,ai,zi,as,sa,d_turb,thetae,cte1,b,d_conv,w_diff
...
10	 format(1x,'Diff. micro. de Proffit et Michaux, dans ZR. ',/,
     1	1x,'H et He4 sont diffuses, les autres sont element trace.',/,
...
c	   ihe4=i		!deja connu
	  ...
	  elseif(nom_elem(i) .eq. 'O17')then
	   zi(i)=8.
	   ai(i)=ao17
	  elseif(nom_elem(i) .eq. ' Z ')then
	   zi(i)=8.
	   ai(i)=17.
	  elseif(nom_elem(i) .eq. ' Ex')then
	   zi(i)=13.
	   ai(i)=28.
	  endif
	 enddo
	 ...
	 if(iz .gt. 1)then
	  write(6,12)zi(iz),ai(iz)
	  write(2,12)zi(iz),ai(iz)	  	
...
c	idee de M.Gabriel : l'expression des coeff de diffusion supposent
c	H et He4 totalement ionises. On suppose si t<10**6K qu'il y a melange
c	de facon a ne pas accumuler de la matiere a la limite
 
	melange=melange .or. t .lt. 1.d6	!ideee de M.Gabriel
	if(melange) then
	 do i=1,nbelem
	  d(2,i)=d_conv
	 enddo
	 ...
	 if(iw .gt. 1)d(2,iw)=w_diff	!moment angulaire	
	
c	 les equations et les derivees numeriques en Xi,
 
	 do k=0,nbelem+1		!0 pour la fonction, nbelem+1 pour ro
	
	  if(k .ne. iw)then	!sauf pour le moment angulaire
	
	   if(k .gt. 0 .and. k .le. nbelem)then	!pour les elements chimiques
	    stor=xchim(k)
	    xchim(k)=stor*unpdx
	    dstor=xchim(k)-stor
	   elseif(k .eq. nbelem+1)then		!derivee / ro
	    stor=ro
	    ro=stor*unpdx
	    dstor=ro-stor
	   endif	
...
c	    dvdx(i) : derivee de v / X1
	    ...
	    if(k .eq. 1)then
	     ...
	     enddo	     	
	     xchim(1)=stor
	    elseif(k .le. nbelem)then	
	     ddx(3,k)=(d(2,k)-ds(2,k))/dstor  !derivee de d(1,i) / Xi
	     xchim(k)=stor
	    elseif(k .eq. nbelem+1)then
	     do i=1,nbelem
	      if(i .ne. iw)then
...
c	    pause
	   ...
	   endif	!pour les fonction ou les derivees
	  endif	!k .ne. iw, ie sauf pour MW
	 enddo	!k derivees numeriques
	
c	 diffusion turbulente
	
	 do i=1,nbelem
	  if(i .eq. iw)then	
	   d(2,i)=d(2,i)+d_turb*m	!pour avoir 0 au centre
	  else
	   d(2,i)=d(2,i)+d_turb
	  endif
	 enddo
