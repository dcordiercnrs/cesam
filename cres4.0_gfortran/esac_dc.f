...
c*************************************************************************
      subroutine esac_dc(xh,ztab,t6,r,iorder,irad,ok)

c Personal version of the original OPAL routine 'esac.f'
c D. Cordier, April 2003, Warsaw.
...
c
      ...
      integer w
      real moles
      logical ok,logic1,logic2 ! D.C., pour faire pareil que Pierre.
     ...
     . ,dfs(nt),dfsr(nr),m,mf,xa(mx)
      ...
      data logic1,logic2/2*.true./ ! D.C., pour faire pareil que Pierre.
...
c      stop
c====================================
c New (Pierre one) version:
62	if(logic1)then
	 write(6,21)slt,xh,t6*1.e6,r,slr
	 write(2,21)slt,xh,t6*1.e6,r,slr
21	 format(1x,/,1x,'!!!!!ATTENTION!!!!!!!!!!',/
     1	1x,'on a recontre au moins 1 fois le Pb. suivant dans la',/,
     2	1x,'routine "esac" de livermore, on a utilise EFF',/,
     3	1x,'T6 ou LogR outside of table range, T6=',1pe10.3,/,
     4	1x,'X=',1pe10.3,' T=',1pe10.3,' ro=',1pe10.3,/,
     5	1x,' LogR=',1pe10.3,/,1x,'!!!!!ATTENTION!!!!!!!!!!',/)		
	 logic1=.false.
         !pause
	endif
	ok=.false.
...
65	if(logic2)then
	 write(6,22)xh,t6*1.e6,r
	 write(2,22)xh,t6*1.e6,r	
...
c
c Version slightly modified by D. Cordier, April 2003, Warsaw.
c
...
c..... The purpose of this subroutine is to read the data tables
      ...
      character*100 fich(8) ! D.C.
      character*100 filename ! D.C.
...
c       call system (' gunzip EOSdata')
      ...
      write(6,*) ' '
      write(6,*) ' ***************************************'
      write(6,*) ' ==>> We are reading the following file: ', 
     &            filename(:long(filename))
      write(6,*) '      (Generally it takes a while!)'
      write(6,*) ' '

      open(2, FILE=filename(:long(filename)))
 
      do 3 m=1,mx

      read (2,'(3x,f6.4,3x,f12.9,11x,f10.7,17x,f10.7)')
     x  xin(m),zz(m),moles(m),tmass(m)
      if ( abs(zz(m)-(1.d0-x0-y0))/zz(m) .gt. 1.d-5) then ! D.C.
         print*, ' *************************************'
         print*, ' ==>> Metallicity read in OPAL file--: ', zz(m)
         print*, ' ==>> Metallicity required by CESAM--: ', 1.d0-x0-y0
         print*, ' *************************************'
         print*, ' '
         stop
      end if
   ...
   13 dfsr(i)=1./(rho(i)-rho(i-1))
      ...
      enddo
c       call system ('gzip EOSdata') ! Removed by D.C.
      write(6,*) ' ==>> Ok! OPAL EOS Table read!'
      write(6,*) ' ***************************************'
      write(6,*) ' '
...
c--------------------------------------------------------------------
     ...
     x   97,96,95,94,93,92/
      ...
      if (itime .ne. 12345678) then
      call esac_dc(0.5,ztab,1.,0.001,1,0,ok)
      endif
...
c     stop
      ...
      call esac_dc(x,ztab,t6,rhog1,1,0,ok)
      p1=eos(1)
        if(p1 .gt. pnr) then
          p2=p1
          rhog2=rhog1
          ...
          call esac_dc(x,ztab,t6,rhog2,1,0,ok)
          p2=eos(1)
        endif
    ...
    1 continue
      ...
      call esac_dc(x,ztab,t6,rhog3,1,0,ok)
      p3=eos(1)
