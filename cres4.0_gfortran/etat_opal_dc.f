c************************************************************************
	...
	subroutine etat_opal_dc(p,t,xchim,deriv,
...
c       Modif. Daniel: si dans coeur céphéide: on utilise EFF
c       c'est le même critère que celui utilisé pour l'opacité.
c       (cf. routine "opal96_tops99_2001.f" par ex.)
	z0=1.d0-x0-y0
	x=xchim(1)
	zi=1.d0-x-xchim(3)-xchim(3-1)
	if( x .le. 1.d-3 .AND. zi .gt. z0 ) then
	   call etat_eff_dc(p,t,xchim,deriv,
     1       ro,drop,drot,drox,drott,drotp,drotx,
     2       u,dup,dut,dux,dutt,dutp,dutx,dh1,dhe1,dhe2,degene)
c	   print*, 'OPAL EOS: p= ', p
c	   print*, '          t= ', t
c	   print*, '      xchim= ', xchim
c	   print*, '         z0= ', z0
c	   print*, '         y0= ', y0
...
c	write(6,2000)p,t
	...
	call esac_dc(x,ztab,t6,r,6,1,ok) ! calc EOS; use r from rhoofp
	if(.not.ok)goto10
...
c	derivees / X	
	...
	if(.not.ok)goto10
        ...
        drox=(r-ro)/dstor
	call esac_dc(x,ztab,t6,r,6,1,ok)
	if(.not.ok)goto10
...
10	call etat_eff_dc(p,t,xchim,deriv,
     1       ro,drop,drot,drox,drott,drotp,drotx,
     2       u,dup,dut,dux,dutt,dutp,dutx,dh1,dhe1,dhe2,degene)
