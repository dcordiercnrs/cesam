...
c**********************************************************************
	...
	subroutine gain_masse(bp,q,n,qt,knot,chim,mc,nc,mct,knotc,mstar,
     1	dt,age,old_m23,new_m23,nm,new_m23t,knotm,etat,nuc)
 
c ---------------------------------------------------------------------
c Modif de "perte_tot.f" : correction de bug pour avoir vraiment un
c gain de masse quand Mtot > 0 dans le fichier *.don3
c
c Daniel Cordier, janvier 2008.
c ---------------------------------------------------------------------
...
c	mais peuvent se produire avec gain de masse
	...
	do while(i .gt. 1)
	 if(m(i) .ge. m(i+1))then
          ! --------------------------------------------------
	  print*,'gain_masse, inversion en',i,nm,m(i),m(i+1)
	  pause
          ! --------------------------------------------------
	  nm=nm-1
	  do l=i,nm
...
c	la perte de masse externe
	...
	old_m23(nm)=old_m23(nm)+mdot*1.d6*dt ! Modif -mdot ->> +mdot
...
c	mais peuvent se produire avec gain de masse
	...
	do while(i .gt. 1)
	 if(old_m23(i) .ge. old_m23(i+1) .or. new_m23(i) .ge. new_m23(i+1))then
          ! ---------------------------------------------------
	  print*,'gain_masse, inversion en',i,nm
          pause
          ! ---------------------------------------------------
