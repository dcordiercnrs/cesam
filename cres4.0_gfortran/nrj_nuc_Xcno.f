c**************************************************************************
	...
	subroutine nrj_nuc_Xcno(t,ro,comp,dcomp,jac,deriv,fait,
     1	epsilon,et,ero,ex,hhe,be7e,b8e,n13e,o15e,f17e)
 

c Routine avec fraction massique totale Xc+Xn+Xo diminuee d'un facteur
c "coeff_deple" (test de l'influence sur l'extension des blue loops via
c le cycle CNO).
...
c	r(30) : reaction Ne22(a,g)Mg26
	...
	real*8 Xc, Xn, Xo, Xcno, Xcno_deple, Delta_Xcno, coeff_deple_C,
     +         coeff_deple_N, coeff_deple_O
...
c#######################################################################
c Calcul des nouvelles fractions massiques du Carbone, de l'Azote et
c de l'oxygene en tenant appliquant de coefficients de depletion

c cf. cahier XV et cahier XVI p. 9.

c D.C., octobre 2000.

c fraction massique du carbone, de l'azote et de l'oxygene :

	Xc=comp(4)*nucleo(4)+comp(5)*nucleo(5)
	Xn=comp(6)*nucleo(6)+comp(7)*nucleo(7)+comp(8)*nucleo(8)
	Xo=comp(9)*nucleo(9)+comp(10)*nucleo(10)+comp(11)*nucleo(11)
...
c le rapport des nombres de particules :
	...
	print*, '#############################################'
	print*, ' '
	print*, '  Valeur de Xcno :', Xcno
        print*, '  (valeur SANS depletion)'
        print*, ' '
	print*, '  (1) Facteur de depletion pour le CARBONE ?'
	print*, '      (on fait Xc ---> Xc/coeff)'
        print*, ' '
	read(5,*) coeff_deple_C
        print*, ' '
	print*, '  (2) Facteur de depletion pour  l''AZOTE ?'
	print*, '      (on fait Xn ---> Xn/coeff)'
        print*, ' '
	read(5,*) coeff_deple_N
        print*, ' '
	print*, '  (3) Facteur de depletion pour  l''OXYGENE ?'
	print*, '      (on fait Xo ---> Xo/coeff)'
        print*, ' '
	read(5,*) coeff_deple_O

        print*, ' '
        print*, ' ===>> voici les coeff. choisis :'
        print*, '  * C : ', coeff_deple_C
        print*, '  * N : ', coeff_deple_N
        print*, '  * O : ', coeff_deple_O
        print*, ' '
        print*, 'Ok ? '
        pause

c Les nouvelles fractions massiques des elements CNO :

	comp(4)= comp(4)/coeff_deple_C ! C12
	comp(5)= comp(5)/coeff_deple_C ! C13

	comp(6)= comp(6)/coeff_deple_N ! N13
	comp(7)= comp(7)/coeff_deple_N ! N14
	comp(8)= comp(8)/coeff_deple_N ! N15

	comp(9) = comp(9)/coeff_deple_O  ! O16
	comp(10)=comp(10)/coeff_deple_O ! O17
	comp(11)=comp(11)/coeff_deple_O ! O18

c Calcul de Xcno apres depletion :
	Xcno_deple= comp(4)*nucleo(4)+comp(5)*nucleo(5)            ! C
     +  +comp(6)*nucleo(6)+comp(7)*nucleo(7)+comp(8)*nucleo(8)      ! N
     +  +comp(9)*nucleo(9)+comp(10)*nucleo(10)+comp(11)*nucleo(11)  ! O

        Delta_Xcno=Xcno-Xcno_deple

        comp(3)=comp(3)+Delta_Xcno/nucleo(3) ! On remet tout sur He4

        y0=comp(2)*nucleo(2)+comp(3)*nucleo(3)

        print*, 'On a donc : '
        print*, 'y0 = ', y0
        print*, 'Xc = ', comp(4)*nucleo(4)+comp(5)*nucleo(5)           ! C  
        print*, 'Xn = ', comp(6)*nucleo(6)+comp(7)*nucleo(7)+comp(8)*
     +                   nucleo(8)                                     ! N
        print*, 'Xo = ', comp(9)*nucleo(9)+comp(10)*nucleo(10)+
     +                   comp(11)*nucleo(11)                           ! O
        print*, 'Ok ?'
        pause
