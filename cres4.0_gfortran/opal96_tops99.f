c--------------------------------------------------------------------------
      ...
      subroutine opal96_tops99(xh,t,ro,kappa,dkapdt,dkapdr,dkapdx)
...
c     ------ calcul des valeurs pour lesquelles on veut l'opacite ---
     ...
     +    abs(z0-0.004d0).ge. limite ) then
         ...
         print*, ' '
         print*, '               Il faut extrapoler'
         print*, ' '
         print*, 'Pour une metalicite proche du SMC il faut utiliser :'
         print*, ' '
         print*, '             <<opal96_tops99_exSMC>>'
         print*, ' '
         print*, '*************************************************'
         print*, ' '
         stop
...
c Sorties: (modification des commentaires, DC Mai 99)
c         kappa (et pas Log10 kappa ) 
c         d kappa / d ro
c         d kappa / d T (a verifier !)
c         d kappa / d X
c
c
c              Yveline (aout 1991-code de Geneve--> 10/95 CESAM)
...
16    continue
      ...
      do 17 i=1,3
        do 70 j=1,3
          ...
          qkl(j)=opat6(i,j)
70      continue
        call quad_yve(x,qx,qk,kapq,dkapq)
        opax(i)=kapq
        opaxx(i)=dkapq
        call quad_yve(x,qx,qkk,kapq,dkapq)
        oparx(i)=kapq
        call quad_yve(x,qx,qkl,kapq,dkapq)
        opat6x(i)=kapq
17    continue
...
19    continue
      ...
      call quad_yve(z,qx,opax,cap,dkapq)
      call quad_yve(z,qx,opaxx,capx,dkapq)
      call quad_yve(z,qx,oparx,capr,dkapq)
      call quad_yve(z,qx,opat6x,capt6,dkapq)
...
c************************************************************************
      ...
      subroutine kappa_tops_99( y, log_do16sdc12, t, ro, file_opa,
     +                         kap, dkapt, dkapro)
...
c			22 Mai 1998
      ...
      parameter( pn_y=10, pn_o16=20, pnt=26, pnro=30)
...
c Lecture des tables dans le fichier 'file_opa'
      ...
      if(i_lect) then
        call lect_tops_99(file_opa,n_y,n_o16,nt,nro,z0,val_y,val_o16,
     +                   vlt6,vlro,vlk)
c      print*, 'vz= ', vz
c      print*, 'vlt6= ', vlt6
c      print*, 'vlro= ', vlro
...
c densite
      ...
      if(lro .lt. vlro(1)) then
        ...
        print*, 'Ro_min (TOPS) = ', 10.**vlro(1)
c        stop
      end if
 
      if(lro .gt. vlro(nro)) then
        print*, 'Pb. avec opacite TOPS, ro= ', ro,' g.cm^-3'
        print*, 'Ro_max (TOPS) = ', 10.**vlro(nro)
c        stop
      end if
...
c temperature
      ...
      if(lt6 .lt. vlt6(1)) then
c        print*, 'Pb. avec opacite TOPS, t= ', t, ' K'
        stop
      end if
...
c-----------------------------------------------------------------------
      ...
      subroutine lect_tops_99(file_opa, n_y, n_o16, nt, nr, z0,
...
c Daniel Cordier, Mai 1998
      ...
      parameter( pnf=200, pnt=26, pnr=30, pn_y=10, pn_o16=20 )
...
c------------------------------------------------------------------------
cc------------------------------------------------------------------------
      subroutine quad_yve(x,qx,qk,k,dk)
