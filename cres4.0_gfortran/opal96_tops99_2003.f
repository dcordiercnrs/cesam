...
c           de passage OPAL -> Los Alamos, de façon à être sûr que les tables de  Los
c           Alamos ne seront utilisées que dans le coeur brûlant de l'He et pas un petit
c           peu -et de façon intermitente- au niveau de la couche brûlant H.
c
c           + Voici le genre de problème qui se produisaient :
c
c                       y    =   0.991987652
c                       zi   =   0.00801149541
c                       xi   =   8.52849563E-07
...
c On cherche les tables a utiliser pour <<TOPS>> :
     ...
     & la valeur de la limite en Z pour laquelle on passe de OPAL à Los 
     & Alamos.'
            print*, ' '
...
c On cherche si on doit utiliser OPAL (tops=.false.) ou TOPS (tops=.true.)
      ...
      if( xi .le. 1.d-2 .AND. zi .gt. z0 * 1.10d0 ) then ! On ajoute 10% à Z0 pour éviter
                                                         ! les problèmes au bord du coeur d'He
c        if ( zi .gt. 0.1 ) then
c           print*, 'y   = ', y
c           print*, 'zi  = ', zi
c           print*, 'xi   = ', xi
