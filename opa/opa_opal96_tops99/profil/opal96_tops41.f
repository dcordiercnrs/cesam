c--------------------------------------------------------------------------
      ...
      subroutine opal96_tops41(xh,t,ro,kappa,dkapdt,dkapdr,dkapdx)
...
c      print*, 'xi=',xi,'zi=',zi,'do16=',do16,'dc12=',dc12

      if( xi .le. 1.d-03 .AND. zi .ge. z0 ) then
        if( dc12 .gt. 0.d0 .AND. do16 .gt. 0.d0 ) then
          log_do16sdc12=log10(do16/dc12)
          tops=.true.
        else
           if(dc12 .gt. 0.d0 .AND. abs(do16) .le. 1.d-3) then
              log_do16sdc12=-4.d0
              tops=.true.
           else
c              print*, 'Pb., xi=',xi,'zi=',zi,'do16=',do16,'dc12=',dc12
c              stop
              y=1.d0-z0
              log_do16sdc12=-0.4d0
              tops=.true.
...
c Interpolation de dlog kap / dlog t6 

      call int_ext_parab(dkroto16,val_y,n_y,y,dkappadro,bidon)
